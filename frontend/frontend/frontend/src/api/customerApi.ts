...
export async function getCustomers(
  ...
  const res = await api.get<ApiPaginatedResponse<Customer>>(BASE, { params });
  return res.data;
}
...
export async function createCustomer(payload: CreateCustomerDto): Promise<Customer> {
  const res = await api.post<ApiResponse<Customer>>(BASE, payload);
  if (!res.data.success || !res.data.data) {
...
export async function updateCustomer(
  ...
  const res = await api.put<ApiResponse<Customer>>(`${BASE}/${id}`, payload);
  if (!res.data.success || !res.data.data) {
