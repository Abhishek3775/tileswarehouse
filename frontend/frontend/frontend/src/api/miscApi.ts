...
export const purchaseOrderApi = {
  getAll: async (params?: PaginationParams): Promise<ApiPaginatedResponse<PurchaseOrder>> => {
  ...
  delete: async (id: string): Promise<ApiResponse<null>> => {
    const res = await axiosInstance.delete<ApiResponse<null>>(`/purchase-orders/${id}`);
    return res.data;
  },
};
...
export const purchaseReturnApi = {
  getAll: async (params?: PaginationParams): Promise<ApiPaginatedResponse<Record<string, unknown>>> => {
    const res = await axiosInstance.get('/purchase-returns', { params });
    return res.data;
  },
  getById: async (id: string): Promise<ApiResponse<Record<string, unknown>>> => {
    const res = await axiosInstance.get(`/purchase-returns/${id}`);
    return res.data;
  },
};
