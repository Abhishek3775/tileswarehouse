...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && (kpis?.lowStockItems !== undefined || kpis?.activePOs !== undefined || summary?.ledgerEntriesLast30Days !== undefined) && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie
                    ...
                    paddingAngle={3}
                  >
                    {stockByCategory.filter((d) => d.boxes > 0).map((_, i) => (
        ...
        <div className="lg:col-span-2 bg-card rounded-lg border shadow-sm">
          <div className="flex items-center justify-between p-4 border-b">
            <h3 className="font-display font-semibold text-foreground">Recent Sales</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-card rounded-lg border shadow-sm">
          <div className="flex items-center justify-between p-4 border-b">
            <h3 className="font-display font-semibold text-foreground">Recent GRNs</h3>
        ...
        <div className="bg-card rounded-lg border shadow-sm">
          <div className="flex items-center justify-between p-4 border-b">
            <h3 className="font-display font-semibold text-foreground">Recent Transfers</h3>
                  ...
                  {recentTransfers.length === 0 ? (
                    ...
                    recentTransfers.map((t) => (
                      <tr key={t.id} className="border-t border-border hover:bg-muted/20 transition-colors">
                        <td className="px-4 py-2.5 font-mono text-xs font-medium text-foreground">{t.transfer_number}</td>
                        <td className="px-4 py-2.5 text-foreground">{t.from_warehouse_name ?? '—'} → {t.to_warehouse_name ?? '—'}</td>
                        <td className="px-4 py-2.5 text-muted-foreground">{formatDate(t.transfer_date)}</td>
                        <td className="px-4 py-2.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-card rounded-lg border shadow-sm">
          <div className="flex items-center justify-between p-4 border-b">
            <h3 className="font-display font-semibold text-foreground">Recent Purchases</h3>
                  ...
                  {recentPurchases.length === 0 ? (
                    ...
                    recentPurchases.map((po) => (
                      <tr key={po.id} className="border-t border-border hover:bg-muted/20 transition-colors">
                        <td className="px-4 py-2.5 font-mono text-xs font-medium text-foreground">{po.po_number}</td>
              ...
              lowStock.map((alert) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground truncate max-w-[140px]">{alert.product_name ?? ''}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-mono font-bold text-destructive">{alert.current_stock_boxes}</p>
