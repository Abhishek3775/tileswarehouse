...
export default function DebitNotesPage() {
  ...
  const { data: prResponse } = useQuery({
    ...
    queryFn: () => purchaseReturnApi.getAll({ page: 1, limit: 200, sortBy: 'created_at', sortOrder: 'DESC' }),
  });
  const purchaseReturns = prResponse?.data ?? [];
  ...
  const fields: FieldDef[] = [
    ...
    { key: 'purchase_return_id', label: 'Purchase Return', type: 'select', options: purchaseReturns.map((p: { id: string; return_number: string }) => ({ value: p.id, label: p.return_number })) },
