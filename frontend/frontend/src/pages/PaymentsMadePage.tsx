...
export default function PaymentsMadePage() {
  ...
  const fields: FieldDef[] = [
    ...
    { key: 'purchase_order_id', label: 'Purchase Order', type: 'select', options: [{ value: '', label: '— None —' }, ...purchaseOrders.map((p) => ({ value: p.id, label: p.po_number }))] },
  ...
  const saveMutation = useMutation({
    mutationFn: async (fd: Record<string, unknown>) => {
      const payload = {
        ...
        purchase_order_id: fd.purchase_order_id ? String(fd.purchase_order_id) : null,
        amount: Number(fd.amount || 0),
        payment_date: fd.payment_date ? String(fd.payment_date).slice(0, 10) : null,
        payment_mode: String(fd.payment_mode || 'bank_transfer'),
  ...
  return (
    <div>
      ...
      <CrudFormDialog open={dialogOpen} onClose={() => { setDialogOpen(false); setEditing(null); }} onSubmit={(d) => saveMutation.mutateAsync(d)} fields={fields} title={editing ? 'Edit Payment' : 'New Payment Made'} initialData={editing ? { ...editing, purchase_order_id: editing.purchase_order_id ?? '' } : undefined} loading={saveMutation.isPending} />
